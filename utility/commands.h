...
/* 0x82 CMD_GET_MASTER_ADDRESS: Get the zigbee address of the master [82]
   Command Format: [CMD] [size] [0x00]
   Expected Response: [0x10] [size] [addr high byte] [addr low byte] [0x11] */
  CMD_GET_MASTER_ADDRESS,

/* 0x83 CMD_GET_NUM_SLAVES: Get the number of slaves in a group [83]
   Command Format: [CMD] [size] [0x00]
   Expected Response [0x10] [size] [1 byte size] [0x11] */
  CMD_GET_NUM_SLAVES,

/* 0x84 CMD_GET_SLAVE_ADDR: Get the address of a group slave [84]
   Command Format: [CMD] [size] [1 byte index] [0x00]
   Expected Response [0x10] [size] [2 byte address, msb first] [0x11] */
  CMD_GET_SLAVE_ADDR,

/* 0x85 CMD_GET_NUM_POSES: Get the number of poses currently recorded [85]
   Command Format: [CMD] [size] [0x00]
   Expected Response [0x10] [size] [1 byte num] [0x00] */
  CMD_GET_NUM_POSES,

/* 0x86 CMD_GET_POSE_DATA: Get the angles for a certain pose [86]
   Command Format: [CMD] [size] [1 byte index] [0x00]
